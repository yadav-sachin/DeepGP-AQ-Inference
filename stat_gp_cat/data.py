...
# !pip install tqdm
#### !git clone https://github.com/Bjarten/early_stopping_pytorch.git
import math
import torch
import gpytorch
...
def return_data(fold,type,scale,data_str):
    train_input = pd.read_csv('../data/beijing-18/'+type+'/fold'+str(fold)+'/train_data' + data_str+'.csv.gz')
    test_input = pd.read_csv('../data/beijing-18/'+type+'/fold'+str(fold)+'/test_data' + data_str+'.csv.gz')
    test_output = pd.read_csv('../data/beijing-18/'+type+'/fold'+str(fold)+'/test_output' + data_str+'.csv.gz')
