...
def get_results(params_dict):
    try:
        os.chdir(base_path)
        script_command = f"python simple.py --num_inducing {params_dict['num_inducing']} --lr {params_dict['lr']} --epochs {params_dict['epoch']} --num_layers {params_dict['num_layer']} --fold {params_dict['fold']}"
